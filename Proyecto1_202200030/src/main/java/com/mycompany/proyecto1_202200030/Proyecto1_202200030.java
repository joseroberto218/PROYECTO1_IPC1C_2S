...
static void generarReportes() {
    ...
    // Validación reporte stock
    if (totalProductos == 0) {
        System.out.println("ERROR AL GENERAR REPORTE DE STOCK: No hay productos registrados");
        registrarBitacora("Generar Reportes", "Error: sin productos");
        return; // Sale de la función si no hay productos
    }

    // Genera reporte de stock en PDF 
    try {
        Document document = new Document();
        PdfWriter.getInstance(document, new FileOutputStream(fechaArchivo + "_Stock.pdf"));
        document.open();
        ...
        document.close();
        System.out.println("REPORTE DE STOCK: " + fechaArchivo + "_Stock.pdf");
        
    } catch (Exception e) {
        System.out.println("ERROR AL GENERAR REPORTE DE STOCK: " + e.getMessage());
        registrarBitacora("Generar Reportes", "Error: problema técnico en stock");
        return;
    }

    // Validación de reporte ventas
    File archivoVentas = new File("ventas.txt");
    if (!archivoVentas.exists() || archivoVentas.length() == 0) {
        System.out.println("NO HAY VENTAS :c");
        registrarBitacora("Generar Reportes", "Error: sin ventas");
        return; // Sale si no hay ventas
    }

    // Genera reporte de ventas en PDF 
    try {
        Document document = new Document();
        PdfWriter.getInstance(document, new FileOutputStream(fechaArchivo + "_Ventas.pdf"));
        document.open();
        ...
        try (BufferedReader br = new BufferedReader(new FileReader("ventas.txt"))) {
            ...
            document.add(new Paragraph("No hay ventas registradas"));
        }
        
        document.close();
        System.out.println("REPORTE DE VENTAS: " + fechaArchivo + "_Ventas.pdf");
        
    } catch (Exception e) {
        System.out.println("PROBLEMAS TECNICOS CON REPORTE DE VENTAS: " + e.getMessage());
        registrarBitacora("Generar Reportes", "Error: problema técnico en ventas");
        return;
    }
